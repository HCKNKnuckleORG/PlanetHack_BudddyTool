...
_LOGIN_PATH_KEYWORDS = ("/admin", "/login", "/wp-admin", "/dashboard", "/panel", "/manager")
...
def _has_dirs_with_ids(findings: Dict[str, Any]) -> bool:
    ...
    for d in dirs:
        ...
        if re.search(r"/\d+/?$", path) or "/user/" in path or "/id/" in path or "/item/" in path:
            return True
    return False
...
for mid in ("sql", "xss", "csrf", "open_redirect", "clickjacking", "file_upload",
            "ssrf", "api", "business_logic", "cache", "information_disclosure",
            "fuzzing", "rce", "template_injection", "deserialization", "xxe",
            "request_smuggling", "session", "a02_misconfiguration", "a05_injection",
            "a06_insecure_design", "a07_auth", "a08_integrity", "a09_logging", "a10_exceptional"):
    if mid not in _READINESS_RULES:
        _READINESS_RULES[mid] = (_web, "Need web target (http(s)) or recon showing web ports (80, 443, 8080...)")
