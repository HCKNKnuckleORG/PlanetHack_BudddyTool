...
def parse_nmap(output: str) -> Dict[str, Any]:
    ...
    for line in output.splitlines():
        port_match = re.match(r"^\s*(\d+)/(tcp|udp)\s+(open|filtered)\s+(.*)$", line)
        if port_match:
            ports.append(
                {
        ...
        if redirect_match:
            ...
            redirect_match = re.search(
                r"Did not follow redirect to (https?://[^\s/]+)", line, re.I
            )
            if redirect_match:
                redirect_hostname = redirect_match.group(1).strip()
...
def _resolve_effective_targets(
    ...
    if redirect_hostname:
        base_url = (
            redirect_hostname
            if redirect_hostname.startswith(("http://", "https://"))
            else redirect_hostname
        )
...
def suggest_next_steps(
    ...
    ip_target, base_url = _resolve_effective_targets(
        findings, target, discovered_hostnames
    )
    ...
    if (
        web_ports
        and "gobuster" not in findings
        and "feroxbuster" not in findings
        and "dirb" not in findings
    ):
        recs.append(
            {
    ...
    if "wordpress" in services_str or any(
        "wordpress" in t.lower() for t in findings.get("whatweb", [])
    ):
        recs.append(
            {
    ...
    dirs = (
        findings.get("gobuster", [])
        or findings.get("feroxbuster", [])
        or findings.get("dirb", [])
    )
    login_dirs = [
        d
        for d in dirs
        if any(
            k in d.get("path", "").lower()
            for k in (
                "/admin",
                "/login",
                "/wp-admin",
...
class ReconReport:
    ...
    def generate_markdown(self) -> str:
        ...
        if nuclei:
            ...
            for f in nuclei:
                lines.append(
                    f"| {f['severity'].upper()} | {f['template']} | {f['detail']} |"
                )
            lines.append("")
        ...
        lines.append("---")
        lines.append(
            f"*Generated by PlanetHack // HCKNKnuckle // {self.end_time.strftime('%Y-%m-%d %H:%M:%S')}*"
        )
        lines.append("")
    ...
    def generate_html(self) -> str:
        ...
        if ports:
            ...
            parts.append(
                "<tr><th>Port</th><th>Proto</th><th>State</th><th>Service</th></tr>"
            )
            for p in ports:
                parts.append(
                    f'<tr><td>{p["port"]}</td><td>{p["proto"]}</td><td>{p["state"]}</td><td>{h(p["service"])}</td></tr>'
                )
            parts.append("</table>")
        ...
        if nikto:
            ...
            for f in nikto[:30]:
                ...
                parts.append(
                    f'<li><span class="{sev_cls}">[{h(f["severity"].upper())}]</span> {h(f["detail"])}</li>'
                )
            parts.append("</ul>")
        ...
        for tool_name in ("gobuster", "feroxbuster", "dirb"):
            ...
            if dirs:
                ...
                for d in dirs[:50]:
                    parts.append(
                        f'<tr><td>{h(d["path"])}</td><td>{d["status"]}</td></tr>'
                    )
                parts.append("</table>")
                break
        ...
        if nuclei:
            ...
            for f in nuclei:
                sev_cls = f"sev-{f['severity']}"
                parts.append(
                    f'<tr><td class="{sev_cls}">{h(f["severity"].upper())}</td><td>{h(f["template"])}</td><td>{h(f["detail"])}</td></tr>'
                )
            parts.append("</table>")
        ...
        parts.append(
            f'<div class="meta">Generated by PlanetHack // HCKNKnuckle // {self.end_time.strftime("%Y-%m-%d %H:%M:%S")}</div>'
        )
