...
def normalize_target(target: str, use_https: bool = False) -> Dict[str, Any]:
    """
...
def build_recon_plan(
    ...
    gobuster_wordlist = shlex.quote(
        kali.get("gobuster_wordlist", f"{wordlist_dir}/dirb/common.txt")
    )
    ...
    if preset in ("full", "htb", "web"):
        threads = (
            50 if preset == "full" else 20
        )  # lighter for CTF/web to avoid VM overload
        gobuster_cmd = (
            f"gobuster dir -u {base_url} -w {gobuster_wordlist} -t {threads} -x {_exts}"
        )
        ferox_cmd = (
            f"feroxbuster -u {base_url} -w {gobuster_wordlist} -t {threads} -x {_exts}"
        )
    ...
    if preset in ("full", "htb", "web") and _hidden_wl.exists():
        ...
        gobuster_hidden_cmd = (
            f"gobuster dir -u {base_url} -w {hidden_wl_quoted} -t {threads} -x {_exts}"
        )
        ferox_hidden_cmd = (
            f"feroxbuster -u {base_url} -w {hidden_wl_quoted} -t {threads} -x {_exts}"
        )
        phases.append(
            {
