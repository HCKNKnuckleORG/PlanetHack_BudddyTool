...
(function (): void {
  ...
  function fetchQuote(): void {
    fetch("/api/quote")
      .then((r) => r.json())
      .then((data: { quote: string; movie: string }) => {
...
/* ── Recon plan builder ──────────────────────────────────────── */
(function (): void {
  const form = document.getElementById("recon-form") as HTMLFormElement | null;
  if (!form) return;
  ...
  form.addEventListener("submit", function (e: Event): void {
    ...
    fetch("/recon/preflight", { method: "POST", body: formData })
      .then((r) => r.json())
      .then(function (pf: PreflightResult) {
  ...
  function showPreflightInfo(msg: string): void {
  ...
  function showPreflightWarnings(warnings: string[]): void {
    if (!planContainer) return;
    warnings.forEach(function (w: string) {
      const div = document.createElement("div");
  ...
  function handleHostsWarning(pf: PreflightResult, formData: FormData): void {
    ...
    if (addBtn) {
      addBtn.addEventListener("click", function (this: HTMLButtonElement) {
        ...
        fetch("/recon/add-host", {
          ...
          .then((r) => r.json())
          .then(function (result: { success: boolean; message?: string }) {
            if (result.success) {
              showPreflightInfo(result.message || "");
              const targetInput = form.querySelector<HTMLInputElement>('input[name="target"]');
  ...
  function buildPlan(formData: FormData): void {
    if (!planContainer) return;
  ...
  function renderPlan(phases: Phase[], target: string): void {
    if (!planContainer) return;
    let html =
    ...
    if (runAll) {
      runAll.addEventListener("click", function () {
        ...
        const presetEl = form.querySelector<HTMLInputElement>('input[name="preset"]:checked');
...
(function (): void {
  ...
  grid.addEventListener("click", function (e: MouseEvent) {
    ...
    const targetInput = document.getElementById("module-target") as HTMLInputElement | null;
    ...
    const body: Record<string, string> = { module_id: moduleId, target: target };
